...
/**
 * Extract the extension directory from hook commands in settings.json.
 * Parses `node "/path/to/extension/hook.js"` to get `/path/to/extension/`.
 */
  ...
  for (const event of ['Stop', 'Notification', 'PermissionRequest']) {
    ...
    for (const entry of hooks[event]) {
      ...
      for (const h of entry.hooks) {
        ...
        if (hookPath) return path.dirname(hookPath);
      }
    }
  }
  return null;
...
function checkHookStatus(extensionPath) {
  ...
  try {
    ...
    if (hasStop && hasNotification && hasPermission) {
      ...
      if (installedPath && path.resolve(installedPath) !== path.resolve(extensionPath)) {
