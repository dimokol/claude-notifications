...
// Each (sessionId) has a stageId that advances on: user prompt, event
// type change, or previous stage resolved. shouldNotify returns
// {notify:false} when the current event is a re-fire of an already-
// notified-and-unresolved stage.
const fs = require('fs');
...
function shouldNotify(workspaceRoot, sessionId, currentEvent) {
  ...
  // Same event type, unresolved → re-fire of an already-notified stage.
